...
毎日03:25 JSTに10分値を確定します。1時間値は毎時35分に、国土交通省の時刻水位月表（前月・当月）から直接取得して既存の年別ファイルへ統合します。取得スクリプトと地点設定は旧リポジトリから浅く取得するだけで、旧リポジトリのGit履歴や大きな `historical_hourly.json` には依存しません。10分値は原本として保持し、30分・1時間などの集約は利用側で行います。
